 ...
 local request_handler = require("paperplanes.curl") local notify_attempt


 local function _3_() local msg = fmt("%s'ing...", get_option("provider"))
 local show = (vim.notify or print)
 return show(msg) end notify_attempt = _3_
 notify_attempt()
 return request_handler(cmd, post_args, provider_cb, final_cb) end

 local function get_buffer_meta(buffer)


 local function _4_() return {path = vim.fn.expand("%:p"), filename = vim.fn.expand("%:t"), extension = vim.fn.expand("%:e"), filetype = vim.bo.filetype} end return vim.api.nvim_buf_call(buffer, _4_) end
 ...
 local _5_ = {url, err} if ((_G.type(_5_) == "table") and ((_5_)[1] == nil) and ((_5_)[2] == err)) then
 return error(("paperplanes got no url back from provider: " .. err)) elseif ((_G.type(_5_) == "table") and ((_5_)[1] == url) and true) then local _ = (_5_)[2]
 local reg = get_option("register") local set_reg
 local function _6_(_241, _242) return (_241 and vim.fn.setreg(_241, _242)) end set_reg = _6_ local notify

 local function _7_(_241, _242) local extra if _242 then extra = fmt("\"%s = ", _242) else extra = "" end
 local msg = fmt("%s%s", extra, _241)
 local via = (vim.notify or print)
 return via(msg) end notify = _7_
 set_reg(reg, url)
 return notify(url, reg) else return nil end end
 return post_range(0, start, stop, maybe_set_and_print) end
