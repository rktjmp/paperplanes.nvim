 ...
 local function execute_request(cmd, args, on_exit) _G.assert((nil ~= on_exit), "Missing argument on-exit on fnl/paperplanes/curl.fnl:4") _G.assert((nil ~= args), "Missing argument args on fnl/paperplanes/curl.fnl:4") _G.assert((nil ~= cmd), "Missing argument cmd on fnl/paperplanes/curl.fnl:4") local io = {stdout = uv.new_pipe(false), stderr = uv.new_pipe(false), output = {}, errput = {}}
 local save_io
 ...
 local function curl(cmd, request_args, provider_cb, final_cb) _G.assert((nil ~= final_cb), "Missing argument final-cb on fnl/paperplanes/curl.fnl:23") _G.assert((nil ~= provider_cb), "Missing argument provider-cb on fnl/paperplanes/curl.fnl:23") _G.assert((nil ~= request_args), "Missing argument request-args on fnl/paperplanes/curl.fnl:23") _G.assert((nil ~= cmd), "Missing argument cmd on fnl/paperplanes/curl.fnl:23")
 ...
 return final_cb(url, err) end on_exit = _11_
 return execute_request(cmd, args, on_exit) end
