...
local function execute_request(args, on_exit)
  _G.assert((nil ~= on_exit), "Missing argument on-exit on fnl/paperplanes/curl.fnl:4")
  _G.assert((nil ~= args), "Missing argument args on fnl/paperplanes/curl.fnl:4")
...
local function curl(request_args, response_handler)
  ...
  on_exit = _11_
  return execute_request(args, on_exit)
end
return curl